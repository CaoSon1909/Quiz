...
public class SearchByNameServlet extends HttpServlet {
     ...
     */
    private int getMaxRecord(int statusValue, String searchValue) throws SQLException, NamingException {
        TblQuestionDAO dao = new TblQuestionDAO();
        return dao.countSearchLikesNameResult(statusValue, searchValue);
    }

    private int getMaxPage(int statusValue, String searchValue) {
        int maxRecord = 0;
        int maxPage = 0;
        try {
            maxRecord = getMaxRecord(statusValue, searchValue);
            maxPage = maxRecord / ROWS_IN_PAGE;
            if (maxRecord % ROWS_IN_PAGE != 0) {
                maxPage += 1;
            }
        } catch (SQLException | NamingException ex) {
            log("SearchByNameServlet:"+ex.getMessage());
        }
        return maxPage;
    }
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            /*Paging*/
            //calculate maxPage
            int maxPage = getMaxPage(status, searchValue);
            //page
            ...
            //setattribute for PREVIOUS_PAGE and NEXT_PAGE
            request.setAttribute("NEXT_PAGE", nextPage);
            request.setAttribute("PRE_PAGE", previousPage);
            //
            TblQuestionDAO dao = new TblQuestionDAO();
            List<TblQuestionDTO> list = dao.searchLikesName(searchValue, status, page);
